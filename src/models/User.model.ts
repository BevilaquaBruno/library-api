...
export default class UserModel {
  ...
  public static async findAll(): Promise<User[]> {
    ...
    Object.values(rows).map((el: UserData) => allUsers.push(new User(el.name, el.username, el.email, el.id)));

    return allUsers;
  }

  public static async find(id: number): Promise<User> {
    const [ rows ] = await (await conn).execute("SELECT id, name, username, email FROM user WHERE id = ?",
    [id.toString()]);
